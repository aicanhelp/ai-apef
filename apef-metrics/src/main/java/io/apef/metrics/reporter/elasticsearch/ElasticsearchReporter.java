...
public class ElasticsearchReporter extends ScheduledReporter {
    ...
    public void report(SortedMap<String, Gauge> gauges,
        ...
        try {
            ...
            List<JsonMetrics.JsonMetric> percolationMetrics = new ArrayList<JsonMetrics.JsonMetric>();
            ...
            for (Map.Entry<String, Counter> entry : counters.entrySet()) {
                JsonMetrics.JsonCounter jsonMetric = new JsonMetrics.JsonCounter(name(prefix, entry.getKey()), timestamp, entry.getValue());
                connection = writeJsonMetricAndRecreateConnectionIfNeeded(jsonMetric, connection, entriesWritten);
                addJsonMetricToPercolationIfMatching(jsonMetric, percolationMetrics);
            }

            for (Map.Entry<String, Histogram> entry : histograms.entrySet()) {
                JsonMetrics.JsonHistogram jsonMetric = new JsonMetrics.JsonHistogram(name(prefix, entry.getKey()), timestamp, entry.getValue());
                connection = writeJsonMetricAndRecreateConnectionIfNeeded(jsonMetric, connection, entriesWritten);
                addJsonMetricToPercolationIfMatching(jsonMetric, percolationMetrics);
            }

            for (Map.Entry<String, Meter> entry : meters.entrySet()) {
                JsonMetrics.JsonMeter jsonMetric = new JsonMetrics.JsonMeter(name(prefix, entry.getKey()), timestamp, entry.getValue());
                connection = writeJsonMetricAndRecreateConnectionIfNeeded(jsonMetric, connection, entriesWritten);
                addJsonMetricToPercolationIfMatching(jsonMetric, percolationMetrics);
            }

            for (Map.Entry<String, Timer> entry : timers.entrySet()) {
                JsonMetrics.JsonTimer jsonMetric = new JsonMetrics.JsonTimer(name(prefix, entry.getKey()), timestamp, entry.getValue());
                connection = writeJsonMetricAndRecreateConnectionIfNeeded(jsonMetric, connection, entriesWritten);
                addJsonMetricToPercolationIfMatching(jsonMetric, percolationMetrics);
            }
    ...
    private HttpURLConnection writeJsonMetricAndRecreateConnectionIfNeeded(JsonMetrics.JsonMetric jsonMetric, HttpURLConnection connection,
