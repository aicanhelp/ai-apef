...
import io.apef.testing.benchmark.BenchmarkRunner;
...
import static io.apef.testing.benchmark.Benchmark.*;
...
//        );
        ...
        benchmark().threads(1).concurrency(20000).iterations(500).rounds(10)
                .async(false)
    ...
    public void runRxjava01() {
        ...
        benchmark().threads(4).concurrency(10000).iterations(500).rounds(10)
                .benchmarkTask((index, runnerContext) -> {
                    observable
                            .doOnError(throwable -> {
                                runnerContext.done(index);
    ...
    public void runRxjava1() {
        ...
        benchmark().concurrency(1).iterations(iterations).rounds(10)
...
//                    .map((Func1<TestEvent, Object>) testEvent -> testEvent)
    ...
    public void runRxjava2() {
        ...
        benchmark().threads(4).concurrency(10000).iterations(iterations).rounds(10)
                .benchmarkTask((index, runnerContext) -> {
                    Observable.just(index)
    ...
    public void runScheduler() {
        ...
        benchmark().concurrency(10000).iterations(500).rounds(10)
                .benchmarkTask((iterations, runnerContext) -> {
                    executor.submit(() -> {
                        runnerContext.done(iterations);
                    });
    ...
    public void runDisruptor1() {
        ...
        disruptor.start();
        benchmark().threads(1).concurrency(1).iterations(10000000).rounds(10)
                .benchmarkTask((index, runnerContext) -> {
    ...
    public void runDisruptor11() {
        ...
        disruptor.start();
        benchmark().threads(1).concurrency(1).iterations(total).rounds(10)
                .benchmarkTask((index, runnerContext) -> {
                    disruptor.publishEvent((testEvent, l, o) -> {
                        testEvent.data(index).runnerContext(runnerContext);
                    }, index);
...
//                })
        ;
        disruptor.start();
        benchmark().threads(4).concurrency(100).iterations(total).rounds(10)
...
//       benchmark(benchmarkConfig(20000, 100000, 100), index -> Observable.just(true));
    ...
    public void runEmptyForTime() {
        benchmark().concurrency(1).iterations(1000000000).rounds(10)
